    ...
    <div class="space-y-6">
      ...
      <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-4 items-center">
        <div class="flex-1 min-w-[200px]">
          <input [(ngModel)]="searchTerm" type="text" placeholder="Procurar por nome ou NUIT..." 
          ...
          <table class="w-full text-left">
            <thead>
              <tr class="bg-gray-50 text-gray-500 text-[10px] uppercase tracking-wider font-semibold select-none">
                <th (click)="toggleSort('name')" class="px-6 py-4 cursor-pointer hover:bg-gray-100 transition-colors">
                  <div class="flex items-center space-x-1">
                    <span>Contribuinte / NUIT</span>
                ...
                <th (click)="toggleSort('province')" class="px-6 py-4 cursor-pointer hover:bg-gray-100 transition-colors">
                  <div class="flex items-center space-x-1">
                ...
                <th (click)="toggleSort('owner_name')" class="px-6 py-4 cursor-pointer hover:bg-gray-100 transition-colors">
                  <div class="flex items-center space-x-1">
                    <span>Subscritor (Dono)</span>
                ...
                <th (click)="toggleSort('plan')" class="px-6 py-4 cursor-pointer hover:bg-gray-100 transition-colors">
                  <div class="flex items-center space-x-1">
                    <span>Plano / Utilizadores</span>
                ...
                <th (click)="toggleSort('usage_30d')" class="px-6 py-4 text-right cursor-pointer hover:bg-gray-100 transition-colors">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr *ngFor="let company of filteredCompanies()" class="hover:bg-gray-50 transition-colors text-sm">
                <td class="px-6 py-4 text-xs font-serif uppercase tracking-tighter">
                  <div class="flex flex-col">
                    <span class="font-medium text-gray-900">{{ company.name }}</span>
                    <span class="text-gray-500 text-xs">NUIT: {{ company.nuit }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 text-gray-600">
                  <div class="flex flex-col">
                    <span class="font-medium text-gray-900">{{ company.province || 'N/A' }}</span>
                    <span class="text-xs text-gray-500">{{ company.district || 'N/A' }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 text-gray-600">
                  <div class="flex flex-col">
                    <span class="font-medium text-gray-900">{{ company.owner_name }}</span>
                ...
                <td class="px-6 py-4">
                  <div class="flex flex-col">
                    ...
                    <span class="text-xs text-gray-500 font-medium">{{ company.user_count }} utilizadores</span>
                  </div>
                </td>
                <td class="px-6 py-4 text-right">
                  <div class="flex flex-col">
                    <span class="font-bold text-gray-900">{{ company.usage_30d | currency:'MZN':'symbol':'1.2-2':'pt-MZ' }}</span>
...
export class AdminCompaniesComponent implements OnInit {
    ...
    try {
      ...
      this.companies.set(data?.map(c => {
        ...
        return {
          ...
          plan: c.subscriptions?.[0]?.plan_name || 'Trial',
  ...
  async openDetails(company: any) {
    ...
    this.selectedCompany.set({
      ...
      invoice_count: (await this.supabase.db.from('invoices').select('id', { count: 'exact' }).eq('company_id', company.id)).count || 0
    });
  }
  ...
  exportCompanies() {
    const headers = ['Nome', 'NUIT', 'Provincia', 'Distrito', 'Proprietario', 'Plano', 'Utilizadores', 'Uso (30d)', 'Faturas (30d)'];
