          ...
          <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div class="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-3">
              <h4 class="text-xs font-bold text-blue-800 uppercase tracking-wider flex items-center">
                ...
                Gestão de Trial / Subscrição
              </h4>
              <div class="flex items-center justify-between">
                <span class="text-xs text-blue-700">Expira em: {{ (editingSub.next_billing | date:'dd/MM/yyyy') || 'N/A' }}</span>
                <button (click)="extendTrial(7)" class="text-[10px] bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700 shadow-sm transition-all font-bold uppercase">
                  +7 Dias
                </button>
              </div>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div *ngFor="let company of selectedSub().companies" class="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                <div class="bg-gray-50/50 p-4 flex justify-between items-center">
                  ...
                  <span class="text-[10px] px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full font-bold uppercase border border-blue-200">
                    {{ company.subscriptions?.[0]?.plan_name || 'Trial' }}
                  </span>
                </div>
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center space-x-3">
                    ...
                    <div class="flex flex-col">
                      ...
                      <span class="text-xs text-gray-500 line-clamp-1 truncate">{{ sub.email }}</span>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4">
...
export class AdminSubscribersComponent implements OnInit {
  ...
  async loadSubscribers() {
    ...
    try {
      // Fetch profiles and their deep hierarchy
        ...
        .select(`
          *,
          companies (
            ...
            subscriptions (plan_name, billing_cycle, next_billing_date, status),
  ...
  extendTrial(days: number) {
    const current = this.editingSub.next_billing ? new Date(this.editingSub.next_billing) : new Date();
    current.setDate(current.getDate() + days);
    this.editingSub.next_billing = current.toISOString().split('T')[0];
  }
  ...
  async saveSubscriber() {
    ...
    // Update subscription next_billing if changed
    if (this.editingSub.companies?.length > 0) {
      const companyId = this.editingSub.companies[0].id;
      await this.supabase.db
        .from('subscriptions')
        .update({ next_billing_date: this.editingSub.next_billing })
